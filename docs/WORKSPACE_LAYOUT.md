...
├── notes/           # Processed knowledge: papers, topics, learning, research
└── memory/          # Agent state: user profile, reading progress, feedback
```
...
- **`papers/sources/pdf/`**: Original PDFs you have legal access to.
- **`papers/sources/markdown/`**: Markdown versions of papers (from MinerU or other extraction tools).
...
- **`notes/learning/`**: Records of your learning process: quizzes, guided review sessions, wrong-answer notes, concept cards.
- **`notes/research/`**: Your original research content: goals, frameworks, reusable conclusions, experiment plans, audit records, knowledge maps, open questions.
...
### Layer 3: `memory/` — Agent state

This layer enables the agent to remember across sessions.

- **`memory/user_profile.md`**: Who you are, what you study, how you prefer to learn.
- **`memory/reading_state.md`**: What you've read, what you're reading, what you've mastered, what needs review.
- **`memory/feedback.md`**: How the agent should improve based on your past interactions.
...
1. **Traceability**: You can always trace a conclusion in `notes/research/` back to a paper note in `notes/papers/` and then to the original PDF in `papers/sources/pdf/`.
